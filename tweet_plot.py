...
if __name__ == "__main__": 
    ...
    time_likes = pd.Series(data=df['likes'].values, index=df['date']) #plotting like data
    time_likes.plot(figsize=(16, 4), label="likes", legend=True)

    time_retweets = pd.Series(data=df['retweets'].values, index=df['date']) #plotting retweet data
